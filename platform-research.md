...
## Análise iptv-org para a separação do Cine

A documentação oficial do [iptv-org/api](https://github.com/iptv-org/api) confirma que `channels.json` fornece país, categorias, nomes alternativos, website e `is_nsfw`; `feeds.json` fornece idiomas, área de transmissão e formato; `logos.json` fornece logos por canal/feed; e `streams.json` fornece URLs live, qualidade, `referrer`, `user_agent` e labels de disponibilidade. A coleção [iptv-org/iptv](https://github.com/iptv-org/iptv) descreve-se como uma coleção de links públicos de canais e orienta a abertura em players live, não como catálogo VOD com filmes atuais ou temporadas.

A implementação deve, portanto, separar dois produtos na UI: **Cine VOD/descoberta**, alimentado por fontes públicas que forneçam ficheiros ou metadata verificáveis, e **TV IPTV ao vivo**, alimentada por streams associados a canais. Angola (`AO`), Brasil (`BR`), Portugal (`PT`) e outros países devem aparecer como filtros reais sobre o campo `country`; género deve usar as categorias oficiais do catálogo, com tradução visual para português. Logos devem ser obtidos de `logos.json` por canal/feed, com `channel.logo` apenas como fallback de compatibilidade e um estado neutro quando a API não tem imagem.

A fonte continua sujeita a labels como `Geo-blocked` e `Not 24/7`, e os links são submetidos publicamente por terceiros; a presença no catálogo não constitui garantia de licença global nem disponibilidade permanente.
