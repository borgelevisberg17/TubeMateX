(() => {
  ...
  function initEntertainment() {
    ...
    const videoPlaybackRate = $('#videoPlaybackRate');
    ...
    function card(item) {
      ...
      const canPlay = !value.metadataOnly && (!value.live || value.streamAvailable) && Boolean(value.url);
      const metadataAction = value.metadataOnly || !canPlay ? 'Abrir fonte' : value.live ? 'Ver direto' : 'Ver agora';
      const facts = [value.country, languageLabel(value), value.quality, value.availabilityLabel, value.feedCount ? `${value.feedCount} feed${value.feedCount === 1 ? '' : 's'}` : '', value.streamCount ? `${value.streamCount} stream${value.streamCount === 1 ? '' : 's'}` : ''].filter(Boolean).slice(0, 4);
      const initials = String(value.channelName || value.title || 'TMX').split(/\s+/).filter(Boolean).slice(0, 2).map(word => word[0]).join('').toUpperCase();
      return `<article class="ent-card" data-item-id="${esc(id)}"><div class="ent-card-art">${value.thumbnail ? `<img class="${value.live ? 'ent-channel-logo' : ''}" src="${esc(value.thumbnail)}" alt="Logo de ${esc(value.channelName || value.title)}" loading="lazy">` : `<div class="ent-card-art-fallback ${value.live ? 'ent-channel-logo-fallback' : ''}">${value.live ? esc(initials) : 'TMX'}</div>`}<span class="ent-card-badge">${esc(value.site)}</span>${value.live ? `<span class="ent-card-live">${value.streamAvailable ? 'AO VIVO' : 'CATÁLOGO'}</span>` : ''}${percent ? `<div class="ent-card-progress"><span style="width:${percent}%"></span></div>` : ''}</div><div class="ent-card-copy"><h4>${esc(value.title)}</h4><p>${esc(value.uploader || (value.live ? 'Canal público' : 'Fonte pública'))}</p>${facts.length ? `<div class="ent-card-facts">${facts.map(fact => `<span>${esc(fact)}</span>`).join('')}</div>` : ''}<div class="ent-card-actions"><button class="primary" type="button" data-action="play" ${value.externalUrl || canPlay ? '' : 'disabled'}>${metadataAction}</button><button type="button" data-action="details" aria-label="Ver detalhes de ${esc(value.title)}">ⓘ</button><button type="button" data-action="list" aria-label="Adicionar ${esc(value.title)} à Minha lista">${listItems().some(entry => keyOf(entry) === id) ? '✓' : '＋'}</button>${value.live || value.metadataOnly ? '' : '<button type="button" data-action="download" aria-label="Descarregar ' + esc(value.title) + '">↓</button>'}</div></div></article>`;
    }
    ...
    function showDetail(item) {
      ...
      if (detailItem.feedCount) detailMeta.insertAdjacentHTML('beforeend', `<span>${detailItem.feedCount} feed${detailItem.feedCount === 1 ? '' : 's'}</span>`);
      if (detailItem.streamCount) detailMeta.insertAdjacentHTML('beforeend', `<span>${detailItem.streamCount} stream${detailItem.streamCount === 1 ? '' : 's'} verificada${detailItem.streamCount === 1 ? '' : 's'}</span>`);
      ...
      detailPlay.textContent = detailItem.metadataOnly ? 'Abrir fonte' : detailItem.live ? (detailItem.streamAvailable ? 'Ver direto' : 'Abrir fonte') : 'Ver agora';
    ...
    async function playItem(item) {
      const value = register(item); if (!value) return;
      if (value.metadataOnly || (value.live && !value.url)) { if (value.externalUrl) window.open(value.externalUrl, '_blank', 'noopener'); else notify('Este item não tem uma fonte externa válida.', 'error'); return; }
      if (!video) return;
      try {
        ...
        const streamCandidates = [{ ...value, url: value.url }, ...(Array.isArray(value.streams) ? value.streams.map(stream => ({ ...value, ...stream })) : [])].filter(stream => {
          if (!stream.url || stream.requiresExternalPlayer || failedStreamUrls.has(stream.url) || seenUrls.has(stream.url)) return false;
        ...
        if (isDash && window.dashjs?.MediaPlayer) { dash = window.dashjs.MediaPlayer().create(); dash.initialize(video, payload.url, false); dash.on(window.dashjs.MediaPlayer.events.ERROR, data => { if (data?.error) { failedStreamUrls.add(currentStreamUrl); dash.reset(); dash = null; const next = (value.streams || []).some(stream => stream.url && !failedStreamUrls.has(stream.url) && !stream.requiresExternalPlayer); if (next) { notify('A fonte DASH falhou; a tentar uma alternativa…', ''); playItem(value); } else showPlayerError('Este stream MPEG-DASH não pôde ser reproduzido. Tenta VLC, mpv ou a fonte original.'); } }); }
        else if (isHls && window.Hls?.isSupported()) { hlsRecoveryAttempts = 0; hls = new window.Hls({ enableWorker: true, lowLatencyMode: false, maxBufferLength: 30, backBufferLength: 30, manifestLoadingMaxRetry: 3, levelLoadingMaxRetry: 3, fragLoadingMaxRetry: 5, manifestLoadingRetryDelay: 1000, levelLoadingRetryDelay: 1000, fragLoadingRetryDelay: 1000 }); hls.loadSource(payload.url); hls.attachMedia(video); hls.on(window.Hls.Events.MANIFEST_PARSED, (_event, data) => { hlsRecoveryAttempts = 0; if (videoLoading) videoLoading.hidden = true; if (videoQuality && Array.isArray(data?.levels) && data.levels.length > 1) { videoQuality.innerHTML = '<option value="-1">Auto</option>' + data.levels.map((level, index) => `<option value="${index}">${level.height ? `${level.height}p` : `${Math.round(Number(level.bitrate || 0) / 1000)} kbps`}</option>`).join(''); videoQuality.hidden = false; } }); hls.on(window.Hls.Events.ERROR, (_event, data) => { if (!data?.fatal) return; if (data.type === window.Hls.ErrorTypes.NETWORK_ERROR && hlsRecoveryAttempts < 3) { hlsRecoveryAttempts += 1; notify(`A reconectar ao stream… (${hlsRecoveryAttempts}/3)`, ''); window.setTimeout(() => hls?.startLoad(-1), 1000 * hlsRecoveryAttempts); return; } if (data.type === window.Hls.ErrorTypes.MEDIA_ERROR && hlsRecoveryAttempts < 3) { hlsRecoveryAttempts += 1; notify(`A recuperar o media… (${hlsRecoveryAttempts}/3)`, ''); hls.recoverMediaError(); return; } failedStreamUrls.add(currentStreamUrl); hls.destroy(); hls = null; if ((value.streams || []).some(stream => stream.url && !failedStreamUrls.has(stream.url) && !stream.requiresExternalPlayer)) { notify('A fonte falhou; a tentar uma alternativa…', ''); playItem(value); } else showPlayerError(hlsFailureMessage(data, value)); }); } else if (isHls && video.canPlayType('application/vnd.apple.mpegurl')) video.src = payload.url;
    ...
    async function loadIptv(append = false) {
      ...
      if (!append) { iptvOffset = 0; iptvTotal = 0; iptvResults.innerHTML = '<div class="ent-loading-state">A validar canais públicos…</div>'; }
      iptvStatus.textContent = append ? 'A carregar mais canais…' : 'A consultar iptv-org…';
      try { const response = await fetch(`/api/iptv/channels?limit=60&offset=${iptvOffset}&q=${encodeURIComponent(query)}&country=${encodeURIComponent(country)}&language=${encodeURIComponent(language)}&category=${encodeURIComponent(category)}`); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || 'Canais indisponíveis.'); const items = (payload.results || []).map(register); if (!append) iptvResults.innerHTML = ''; if (!items.length && !append) iptvResults.innerHTML = empty('Nenhum canal aprovado corresponde aos filtros.'); else iptvResults.insertAdjacentHTML('beforeend', items.map(card).join('')); iptvTotal = Number(payload.total || 0); iptvOffset += items.length; iptvStatus.textContent = `${iptvOffset} de ${iptvTotal} canais públicos · origem iptv-org`; const more = $('#loadMoreIptv'); if (more) { more.hidden = !Boolean(payload.hasMore); more.textContent = `Carregar mais canais (${Math.min(60, Math.max(0, iptvTotal - iptvOffset))} disponíveis)`; } notify(`${iptvOffset} canais IPTV públicos carregados.`, items.length ? 'success' : ''); }
      catch (error) { if (!append) iptvResults.innerHTML = empty(error.message || 'Canais IPTV indisponíveis.'); iptvStatus.textContent = 'Consulta indisponível'; notify(error.message || 'Canais IPTV indisponíveis.', 'error'); }
    }
    async function loadSources() { const box = $('#iptvSourceList'); if (!box) return; try { const [playlistResponse, catalogResponse] = await Promise.all([fetch('/api/iptv/playlists'), fetch('/api/entertainment/sources')]); const playlists = await playlistResponse.json(); const catalog = await catalogResponse.json(); const playlistTags = (playlists.sources || []).map(source => `<span class="iptv-source-tag ${esc(source.safety)}" title="${esc(source.note)}">${esc(source.label)} · ${esc(source.safety === 'filtered' ? 'filtrada' : source.safety === 'blocked' ? 'bloqueada' : 'não verificada')}</span>`).join(''); const catalogTags = (catalog.sources || []).map(source => source.url ? `<a class="iptv-source-tag catalog-source-tag" href="${esc(source.url)}" target="_blank" rel="noopener" title="${esc(source.policy || '')}">${esc(source.label)} · ${esc(source.mode || 'metadata')}${source.configured === false ? ' · opcional' : ''}</a>` : `<span class="iptv-source-tag catalog-source-tag" title="${esc(source.policy || '')}">${esc(source.label)} · ${esc(source.mode || 'metadata')}${source.configured === false ? ' · opcional' : ''}</span>`).join(''); box.innerHTML = `<div class="iptv-source-heading"><strong>Fontes auditadas</strong><span>${esc(catalog.policy || playlists.policy || '')}</span></div>${playlistTags}${catalogTags}`; } catch { box.textContent = 'Estado das fontes indisponível; nenhuma lista externa foi importada.'; } }
    async function loadIptvMeta() { try { const response = await fetch('/api/iptv/meta'); if (!response.ok) return; const payload = await response.json(); const country = $('#iptvCountry'); const category = $('#iptvCategory'); const countryNames = { AO: 'Angola', BR: 'Brasil', PT: 'Portugal', US: 'Estados Unidos', GB: 'Reino Unido', KR: 'Coreia do Sul', JP: 'Japão', ES: 'Espanha', FR: 'França', DE: 'Alemanha', MZ: 'Moçambique', CV: 'Cabo Verde' }; const categoryNames = { news: 'Notícias', movies: 'Filmes', series: 'Séries', documentary: 'Documentários', cooking: 'Cozinha', music: 'Música', religious: 'Religioso', kids: 'Infantil', animation: 'Anime/animação', sports: 'Desporto', general: 'Generalista', entertainment: 'Entretenimento', culture: 'Cultura', education: 'Educação' }; if (country) country.innerHTML = '<option value="">Todos os países</option>' + (payload.countries || []).map(item => `<option value="${esc(item.code)}">${esc(countryNames[item.code] || item.code)} · ${item.count}</option>`).join(''); if (category) category.innerHTML = '<option value="">Todos os géneros</option>' + (payload.categories || []).map(item => `<option value="${esc(item.id)}">${esc(categoryNames[item.id] || item.id)} · ${item.count}</option>`).join(''); } catch {} }
    async function search(event) {
      ...
      try { let payload; if (/^https?:\/\//i.test(term)) { const response = await fetch(`/api/media/info?url=${encodeURIComponent(term)}`); payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || 'Não foi possível resolver este URL.'); payload = { results: [{ ...payload, url: term, kind: 'film' }] }; } else { const response = await fetch(`/api/entertainment/search?q=${encodeURIComponent(term)}`); payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || 'A pesquisa falhou.'); } const items = (payload.results || []).map(register); searchResults.innerHTML = items.length ? items.map(card).join('') : empty('Nenhum resultado real para esta pesquisa.'); notify(`${items.length} resultados encontrados.`, items.length ? 'success' : ''); searchZone.scrollIntoView({ behavior: 'smooth', block: 'start' }); }
    ...
    $('#loadIptv')?.addEventListener('click', () => loadIptv(false));
    $('#loadMoreIptv')?.addEventListener('click', () => loadIptv(true));
    ['iptvQuery', 'iptvCountry', 'iptvLanguage', 'iptvCategory'].forEach(id => { const control = $(`#${id}`); control?.addEventListener('change', () => { if (id !== 'iptvQuery') loadIptv(false); }); });
    $('#iptvQuery')?.addEventListener('keydown', event => { if (event.key === 'Enter') loadIptv(false); });
    ...
    video?.addEventListener('error', () => { if (currentItem?.live && currentStreamUrl) { failedStreamUrls.add(currentStreamUrl); const next = (currentItem.streams || []).some(stream => stream.url && !failedStreamUrls.has(stream.url) && !stream.requiresExternalPlayer); if (next) { notify('A fonte falhou; a tentar uma alternativa…', ''); playItem(currentItem); return; } } showPlayerError('A fonte não pôde ser reproduzida neste browser. Tenta VLC, mpv ou a fonte original.'); });
    ...
    videoQuality?.addEventListener('change', () => { if (hls) hls.currentLevel = Number(videoQuality.value); });
    videoPlaybackRate?.addEventListener('change', () => { if (video) video.playbackRate = Number(videoPlaybackRate.value || 1); });
    videoPip?.addEventListener('click', async () => { if (!video || !document.pictureInPictureEnabled) return; try { if (document.pictureInPictureElement) await document.exitPictureInPicture(); else await video.requestPictureInPicture(); } catch { notify('Picture-in-Picture não está disponível para esta fonte.', 'error'); } });
    ...
    ytPlayerStage?.addEventListener('keydown', event => { if (!videoDrawer || videoDrawer.hidden || /input|select|button|textarea/i.test(event.target?.tagName || '')) return; if (event.key === ' ') { event.preventDefault(); togglePlay(); } if (event.key === 'ArrowLeft') { event.preventDefault(); video.currentTime = Math.max(0, video.currentTime - 10); } if (event.key === 'ArrowRight') { event.preventDefault(); if (Number.isFinite(video.duration)) video.currentTime = Math.min(video.duration, video.currentTime + 10); } if (event.key.toLowerCase() === 'm') { video.muted = !video.muted; if (videoMute) videoMute.textContent = video.muted ? '🔇' : '🔊'; } if (event.key.toLowerCase() === 'f') videoFullscreen?.click(); });
    ...
    loadSources(); loadHome(); loadIptvMeta().then(() => loadIptv());
  }
