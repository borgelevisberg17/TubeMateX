(() => {
  ...
  function initEntertainment() {
    ...
    function card(item) {
      ...
      const metadataAction = value.metadataOnly ? 'Abrir fonte' : value.live ? (value.streamAvailable ? 'Ver direto' : 'Abrir fonte') : 'Ver agora';
    ...
    async function playItem(item) {
      const value = register(item); if (!value) return;
      if (value.metadataOnly || (value.live && !value.url)) { window.open(value.externalUrl || 'https://iptv-org.github.io/', '_blank', 'noopener'); return; }
      if (!video) return;
      try {
