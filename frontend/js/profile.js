(() => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    $('#downloadHistory').addEventListener('click', async event => { const action = event.target.closest('[data-history-action]'); if (!action) return; const id = action.dataset.historyId; const item = state.lastItems?.find(entry => entry.id === id); if (action.dataset.historyAction === 'details' && item) { notify(`${item.title || 'Ficheiro'} · ${item.formatLabel || item.format || 'formato'} · ${item.site || 'plataforma'}`, 'success'); return; } if (action.dataset.historyAction === 'remove') { if (!confirm('Remover este item do histórico e apagar o ficheiro?')) return; const response = await fetch(`/api/history/${encodeURIComponent(id)}`, { method: 'DELETE' }); if (!response.ok) { notify('Não foi possível remover o item.', 'error'); return; } notify('Item removido do histórico.'); await loadLibrary(); loadUserAndStats(); } });
