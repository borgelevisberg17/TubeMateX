(() => {
  ...
  function openDetails(item, playNow = false) {
    ...
    const cover = item.thumbnail ? `<img src="${esc(item.thumbnail)}" alt="" />` : '<span class="history-detail-cover-fallback">TMX</span>';
    const streamLabel = playable ? (isAudio ? 'Stream de áudio pronta' : 'Stream de vídeo pronta') : 'Stream não disponível';
    body.innerHTML = `<div class="history-detail-hero"><div class="history-detail-cover">${cover}</div><div class="history-detail-summary"><span class="history-detail-platform">${esc(item.site || 'Fonte pública')}</span><h3>${esc(item.title || 'Download')}</h3><p>${esc(item.uploader || item.site || 'Conteúdo descarregado')}</p><span class="history-detail-status ${playable ? 'is-ready' : ''}">${streamLabel}</span></div></div>${playable ? `<div class="history-player">${isAudio ? `<audio controls ${playNow ? 'autoplay' : ''} src="${esc(item.downloadUrl)}"></audio>` : `<video controls playsinline ${playNow ? 'autoplay' : ''} src="${esc(item.downloadUrl)}"></video>`}</div>` : '<p class="history-detail-note">Este item ainda não tem um ficheiro reproduzível associado.</p>'}<div class="history-detail-grid"><div><span>Formato</span><strong>${esc(item.formatLabel || item.format || 'Ficheiro')}</strong></div><div><span>Plataforma</span><strong>${esc(item.site || '—')}</strong></div><div><span>Concluído</span><strong>${date(item.completedAt || item.createdAt)}</strong></div><div><span>Tamanho</span><strong>${item.size ? `${Math.max(1, Math.round(item.size / 1024 / 1024))} MB` : '—'}</strong></div></div><div class="history-detail-actions">${item.downloadUrl ? `<a class="u-action primary" href="${esc(item.downloadUrl)}" download>Guardar ficheiro</a>` : ''}<button class="u-action" type="button" id="closeHistoryDetailsInline">Fechar</button></div>`;
