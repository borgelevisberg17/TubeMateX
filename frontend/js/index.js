(() => {
  ...
  async function loadPlatforms() {
    ...
    try { const response = await fetch('/api/platforms'); if (!response.ok) throw new Error(); const payload = await response.json(); const platforms = payload.platforms || []; const pluginCount = payload.plugins?.length || 0; $('#platformsSummary').textContent = `${platforms.length} plataformas reconhecidas pelo backend${pluginCount ? ` · ${pluginCount} plugin(s) ativo(s)` : ' · plugins opcionais não configurados'}`; grid.innerHTML = platforms.map(platform => `<article class="platform-card ${platform.mode === 'metadata-only' ? 'is-limited' : ''}"><div class="platform-card-top"><strong>${escapeHtml(platform.label)}</strong><span class="platform-status">${platform.mode === 'metadata-only' ? 'Catálogo' : 'URL direta'}</span></div><p>${escapeHtml(platform.note || (platform.mode === 'metadata-only' ? 'Disponível para metadata; não é download direto.' : 'Download quando a URL pública for compatível.'))}</p><button type="button" class="platform-use" data-platform="${escapeHtml(platform.id)}">Usar link</button></article>`).join(''); }
