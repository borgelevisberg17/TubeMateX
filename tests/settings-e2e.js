...
(async () => {
  const browser = await chromium.launch({ headless: true }); const page = await browser.newPage({ viewport: { width: 900, height: 900 } }); const errors = [];
  ...
  assert.equal(await page.locator('#defaultFormat').count(), 1); assert.equal(await page.locator('#autoplayToggle').count(), 1);
