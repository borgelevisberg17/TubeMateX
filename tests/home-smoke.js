...
(async () => {
  ...
  try {
    for (const width of [1440, 768, 390]) {
      ...
      for(const selector of ['#searchForm','#searchResults','#downloadForm','#videoUrl','#quickFormat','#quickQuality','#liveQueue','.tmx-space-footer'])if(await page.locator(selector).count()!==1)throw new Error(`home sem ${selector}`);
