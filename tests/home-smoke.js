...
(async () => {
  ...
  try {
    for (const width of [1440, 768, 390]) {
      ...
      const response=await page.goto('http://localhost:3000/',{waitUntil:'networkidle'}); if(!response||!response.ok())throw new Error(`home falhou em ${width}px`);
      for(const selector of ['#searchForm','#searchResults','#downloadForm','#videoUrl','#quickFormat','#quickQuality','#liveQueue'])if(await page.locator(selector).count()!==1)throw new Error(`home sem ${selector}`);
